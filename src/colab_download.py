...
for t in T:
    try:
        d = yf.download(t, start="2022-01-01", end="2026-09-30", auto_adjust=True, progress=False, threads=False)
        c = d["Close"]
...
pd.DataFrame(log, columns=["ticker", "rows"]).to_csv("download_log.csv", index=False)
files.download("prices_daily.csv"); files.download("download_log.csv")
